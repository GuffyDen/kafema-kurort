...
const sessionCookieName = "tablo_barista_session";
...
type AuthorizationResult =
  | { ok: true; session: BaristaSession }
...
export class BaristaAuthStorageError extends Error {}

export async function authenticateBaristaCredentials(
  username: string,
  password: string,
) {
  if (
  ...
  try {
    account = await getAuthAccount("barista");
  } catch {
    throw new BaristaAuthStorageError("Barista account storage is unavailable.");
  }
...
export async function consumeBaristaLoginAttempt(
  request: Request,
  ...
  const fingerprint = createHash("sha256")
    ...
    .digest("hex");
  const key = `tablo:tenant:${tenantId}:barista-login-rate:v1:${fingerprint}`;
  ...
  try {
    ...
    if (persistence.mode === "redis") {
      ...
      throw new BaristaAuthStorageError("Barista auth storage is not configured.");
    }

    if (!Number.isFinite(count)) {
      throw new BaristaAuthStorageError("Invalid rate limit response.");
    }
    ...
    if (error instanceof BaristaAuthStorageError) throw error;
    throw new BaristaAuthStorageError("Barista auth storage is unavailable.");
  }
}

export async function clearBaristaLoginAttempts(key: string) {
  const persistence = getStorefrontPersistence();
  try {
    if (persistence.mode === "redis") {
...
export async function createBaristaSession(account: AuthAccount) {
  if (account.role !== "barista") {
    throw new BaristaAuthStorageError("A Barista account is required.");
  }
  const tenantId = getTenantId();
  const session: BaristaSession = {
    role: "barista",
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const key = sessionKey(tenantId, token);
    try {
      if (persistence.mode === "redis") {
        const result = await executeRedisCommand([
          "SET",
  ...
  throw new BaristaAuthStorageError("Could not create a unique barista session.");
}

export async function authorizeBaristaRequest(
  request: Request,
): Promise<AuthorizationResult> {
  const token = getBaristaSessionToken(request);
  if (!token) {
    return { ok: false, status: 401, message: "Требуется вход бариста." };
  }

  try {
    const session = await readBaristaSession(token);
    return session
      ? { ok: true, session }
      : { ok: false, status: 401, message: "Требуется вход бариста." };
  } catch {
    return {
      ok: false,
      status: 503,
      message: "Авторизация бариста временно недоступна.",
    };
  }
}

export async function hasBaristaSession(token: string | undefined) {
  if (!token) return false;
  try {
    return Boolean(await readBaristaSession(token));
  } catch {
    return false;
  }
}

export async function destroyBaristaSession(request: Request) {
  const token = getBaristaSessionToken(request);
  if (!token) return;
  const tenantId = getTenantId();
  const key = sessionKey(tenantId, token);
...
function getBaristaSessionToken(request: Request) {
...
async function readBaristaSession(token: string) {
...
function parseSession(value: string): BaristaSession | null {
  try {
    const session = JSON.parse(value) as Partial<BaristaSession>;
...
function sessionKey(tenantId: string, token: string) {
  const tokenHash = createHash("sha256").update(token).digest("hex");
  return `tablo:tenant:${tenantId}:barista-session:v1:${tokenHash}`;
}
